SUBJ_NAMES = {
    "10 гум": {
        'd1': 'Второй иностранный язык (Б)',
        'd2': 'География (Б)',
        'd3': 'Обществознание (Б)',
        'd4': 'Теория познания (Б)',
        'd5': 'Информатика (Б)',
        'd6': 'Естествознание (Б)',
        'd7': 'Диалоги с Чеховым',
        'd8': 'Современная литература в жанровом аспекте',
        'd9': 'Риторика',
        'd10': 'История философии',
        'd11': 'История мировой культуры',
    },
    "10 соцгум": {
        'd1': 'География (Б)',
        'd2': 'Теория познания (Б)',
        'd3': 'МХК (Б)',
        'd4': 'Информатика (Б)',
        'd5': 'Естествознание (Б)',
        'd6': 'Зарубежная литература',
        'd7': 'Современная литература в жанровом аспекте',
        'd8': 'Диалоги с Чеховым',
        'd9': 'Политология',
        'd10': 'Социология',
        'd11': 'История философии',
    },
    "10 соцэк": {
        'd1': 'Теория познания (Б)',
        'd2': 'МХК (Б)',
        'd3': 'География (Б)',
        'd4': 'Информатика (Б)',
        'd5': 'Естествознание (Б)',
        'd6': 'Экономическая география',
        'd7': 'История философии',
        'd8': 'Сложные математические задачи',
    },
    '10 матинф': {
        'd1': 'Обществознание (Б)',
        'd2': 'География (Б)',
        'd3': 'МХК (Б)',
        'd4': 'Теория познания (Б)',
        'd5': 'Биология (Б)',
        'd6': 'Физика (Б)',
        'd7': 'Химия (Б)',
        'd8': 'Избранные главы учебников естественных наук',
    },
    '10 физмат': {
        'd1': 'Обществознание (Б)',
        'd2': 'География (Б)',
        'd3': 'МХК (Б)',
        'd4': 'Теория познания (Б)',
        'd5': 'Биология (Б)',
        'd6': 'Химия (Б)',
        'd7': 'Астрономия',
        'd8': 'Избранные главы учебников естественных наук',
    },
    '10 физтех': {
        'd1': 'Обществознание (Б)',
        'd2': 'География (Б)',
        'd3': 'МХК (Б)',
        'd4': 'Теория познания (Б)',
        'd5': 'Биология (Б)',
        'd6': 'Химия (Б)',
        'd7': 'Инженерная графика (геометрическое и проекционное черчение)',
        'd8': 'Астрономия',
        'd9': 'Избранные главы учебников естественных наук',
    },
    '10 хим': {
        'd1': 'Обществознание (Б)',
        'd2': 'География (Б)',
        'd3': 'МХК (Б)',
        'd4': 'Теория познания (Б)',
        'd5': 'Информатика (Б)',
        'd6': 'Биология (Б)',
        'd7': 'Решение сложных задач по математике',
        'd8': 'Физические задачи повышенной сложности',
        'd9': 'Биологические задачи',
        'd10': 'Химический практикум',
        'd11': 'Астрономия',
    },
    '10 био': {
        'd1': 'Обществознание (Б)',
        'd2': 'География (Б)',
        'd3': 'МХК (Б)',
        'd4': 'Теория познания (Б)',
        'd5': 'Информатика (Б)',
        'd6': 'Физика (Б)',
        'd7': 'Решение сложных задач по математике',
        'd8': 'Информатика в задачах',
        'd9': 'Сложные вопросы ботаники, зоологии, анатомии',
        'd10': 'Химические задачи повышенной сложности',
        'd11': 'Биологический практикум',
    },
}