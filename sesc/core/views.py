...
def export(request):
    ...
    for f_name, full_data in get_subjects().items():
        data, all_records = full_data
        sh = BOOK.create_sheet(f_name)
        # Merging
        sh.merge_cells('A2:A3')
        sh.merge_cells('B1:B3')
        sh.merge_cells(start_row=1, end_row=1, start_column=3, end_column=2 + len(data[0]) + len(data[1]))
        ...
        sh.merge_cells(start_row=2, end_row=2, start_column=3 + len(data[0]),
                       end_column=2 + len(data[0]) + len(data[1]))
        # Headers
        cell_center_ptserif((
        ...
        cell_borders(summ([[sh.cell(i, 3 + j) for j in range(len(data[0]) + len(data[1]))] for i in (2, 3)]))
        # Add subjects
        ind, COLORS = 2, ('E1EFDA', 'FFF2CD', 'FFBF00')
        ...
        # Other styles
        col_width([sh.column_dimensions[ALPH[(2 + i) % len(ALPH)]] for i in range(len(data[0]) + len(data[1]))])
